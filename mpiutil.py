...
copied from https://github.com/radiocosmology/caput
"""
...
def enable_mpi_exception_handler():
    ...
    try:
        ...
        logger.debug('Could not enable faulthandler. ' 'This often happens when running within a test suite.')
...
def parallel_map(func, glist, root=None, method='con', comm=_comm):
    ...
    if rlist is not None:
        ...
        return [item for _, item in sortlist]
    else:
        return None
...
def gather_local(global_array, local_array, local_start, root=0, comm=_comm):
    ...
    if comm is None or comm.size == 1:
        ...
        if comm.rank == root:
            ...
            sub_type = [
                mpi_type.Create_subarray(global_array.shape, local_sizes[i], local_starts[i]).Commit()
                for i in nonempty_procs
            ]  # default order=ORDER_C
            # Post each receive
            reqs = [
                comm.Irecv([global_array, sub_type[si]], source=sr, tag=0) for (si, sr) in enumerate(nonempty_procs)
            ]
        ...
        if (np.prod(local_size) > 0) and (sreq is not None):
            sreq.Wait()
...
def transpose_blocks(row_array, shape, comm=_comm):
    ...
    for ir, ic, request in requests_send:
        ...
        if stat.error != MPI.SUCCESS:
            logger.error('**** ERROR in MPI SEND (r: %i c: %i rank: %i) *****', ir, ic, comm.rank)
    ...
    for ir, ic, request in requests_recv:
        ...
        if stat.error != MPI.SUCCESS:
            logger.error('**** ERROR in MPI RECV (r: %i c: %i rank: %i) *****', ir, ir, comm.rank)
...
def parallel_rows_write_hdf5(fname, dsetname, local_data, shape, comm=_comm):
    ...
    offset, _ = allocate_hdf5_dataset(fname, dsetname, shape, local_data.dtype, comm=comm)
...
class MPILogFilter(logging.Filter):  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        ...
        comm: T.Optional['MPI.Intracomm'] = None,
    ):
